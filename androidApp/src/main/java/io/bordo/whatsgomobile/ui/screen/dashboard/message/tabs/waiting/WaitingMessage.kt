package io.bordo.whatsgomobile.ui.screen.dashboard.message.tabs.waiting
...
fun WaitingMessageScreen(
    ...
    LazyColumn {
        items(items = playerList.playerList) { user ->
            WaitingMessageView(user, isDataLoading, onMessageSelected)
        }
    }
}

@Composable
private fun WaitingMessageView(
    user: Player,
    ...
    Row(modifier = Modifier
        .placeholder(
            visible = isDataLoading, color = lowfidelitygray
        )
        .clickable { onMessageSelected(user.id) }) {
...
private fun leftSection(
    user: Player, isDataLoading: Boolean
) {
    Box(
...
private fun centerSection(user: Player) {
    Column(
        ...
        Text(text = user.name)
        Text(
            text = "Merhabalar, 2 adet sipariş vermek ...", fontSize = 10.sp
        )
    }
}
...
private fun rightSection(user: Player) {
    Column(
        ...
        Text(
            text = "20.02.22", fontSize = 10.sp, color = Color.LightGray
        )
        ...
        Box(
            contentAlignment = Alignment.Center, modifier = Modifier.background(
                color = BtnGreenLight, shape = CircleShape
            )
        ) {
            Text(
                text = user.team, fontSize = 10.sp, modifier = Modifier.padding(horizontal = 3.dp)
            )
        }
    }
}
